/*******************************************************************************
 * File Name    : main.c
 * Description  : Complete FSM Conveyor System (100% Non-Blocking)
 * Hardware     : HW-480 Module (PB8 = Red/Reject)
 * Mode         : IR -> LDR Sequence + Ghost LDR Counter (No Status LED)
 ******************************************************************************/
...
/* --- Configuration --- */
#define REJECT_LED_ACTIVE_LOW   0     // 0 = Active High (Common Cathode)
#define LDR_THRESHOLD_DARK      2000  // ค่า Threshold สำหรับแยกแสง/มืด LDR
#define LDR_DARK_WHEN_HIGH      1     // 1 = มืดแล้วค่า ADC สูงขึ้น
...
volatile uint16_t adc_buffer[2]; // DMA Buffer: [0]=LDR, [1]=POT
...
volatile uint32_t last_btn_start = 0, last_btn_pause = 0, last_btn_reset = 0;
...
int main(void)
{
    ...
    while (1)
    {
        // 1. จัดการดับไฟแสดงผลลัพธ์ (LEDs) เมื่อครบกำหนดเวลา (Non-blocking)
        ...
        switch (current_state)
        {
            ...
            case STATE_RUNNING:
                ...
                // --- ตรวจจับ Ghost LDR (นับอย่างเดียว ไร้ไฟแสดงผล) ---
                if (IS_LDR_DARK()) {
                    if ((msTicks - last_ldr_ghost_trigger) > LDR_COOLDOWN_MS) {
                        last_ldr_ghost_trigger = msTicks;
                        err_ghost_ldr++;
                        UART2_TxString("-> [ERROR: GHOST_LDR] LDR triggered directly without IR!\r\n");
                    }
                }
                break;
...
void System_Init(void) {
    ...
    // 3. Analog Inputs (PA1 = LDR, PA4 = POT)
    GPIOA->MODER |= (3 << (LIGHT_SENSOR_PIN * 2)) | (3 << (POT_PIN * 2));

    // 4. Digital Inputs Setup (PB3, PB4, PB5, PC10)
    ...
    // 7. DMA2 Stream 0 Channel 0 Setup
    DMA2->LIFCR = 0x3D;
    DMA2_Stream0->CR = 0;
    ...
    // 8. ADC1 Setup (Scan Continuous DMA Mode)
